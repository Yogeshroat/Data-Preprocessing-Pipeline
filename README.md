...
Cleans company names (removes extra info, parentheticals)
Uses a mapping of known company names to domains
Attempts to construct likely domains from company names
Validates websites by checking HTTP response
Records the source URL for each validation
...
Extracts domain from company website URL
Normalizes executive name (removes nicknames, quotes, etc.)
Generates both email patterns
Documents patterns used in email_patterns_used.csv
Task 5: Quality Check
Checks Performed:

Ensures all required columns exist
Filters entries missing critical information
Ensures exactly 50 executives (or as many as available)
Validates email format consistency
Checks domain consistency between website and emails
...
Show the complete workflow running from start to finish
Demonstrate each task's execution and output
Show validation of company websites (Task 3) and employment verification (Task 3b)
Show email generation process (Task 4) and patterns tried log
Show quality check results (Task 5) including tier composition
Display the final output CSV file and logs in outputs/logs/workflow.log
Verify all 50 executives are present (or document shortfall and why) with complete data
Troubleshooting
